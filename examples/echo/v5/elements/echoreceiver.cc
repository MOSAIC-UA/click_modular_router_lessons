...
CLICK_DECLS
EchoReceiver::EchoReceiver(): sender(0), infoBase(0)
{
...
int EchoReceiver::configure(Vector<String> &conf, ErrorHandler *errh) {
	if (cp_va_kparse(conf, this, errh, "SENDER", cpkM, cpElement, &sender, "INFOBASE", cpkN, cpElement, &infoBase, cpEnd) < 0) return -1;
	return 0;
}
...
void EchoReceiver::push(int, Packet *p)
{
	...
	if (toPrt == 7) {
		click_chatter("received ECHO request with content %s from %s:%u", content.c_str(), from.unparse().c_str(), fromPrt);
//		sender->generate(content, IPAddress(ipHdr->ip_src), ntohs(udpHdr->uh_sport), 7);
	} else {
		if ((infoBase && infoBase->removeRequest(from, toPrt)) || !infoBase) {
			click_chatter("received ECHO reply with content %s from %s:%u", content.c_str(), from.unparse().c_str(), fromPrt);
		}
	}
