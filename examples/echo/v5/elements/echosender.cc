...
CLICK_DECLS
EchoSender::EchoSender(): infoBase(0)
{}
...
int EchoSender::configure(Vector<String> &conf, ErrorHandler *errh) {
	if (cp_va_kparse(conf, this, errh, "MAXPACKETSIZE", cpkM, cpInteger, &maxSize, "INFOBASE", cpkN, cpElement, &infoBase, cpEnd) < 0) return -1;
...
void EchoSender::generate(const String &content, const IPAddress &to, uint16_t toPort, uint16_t fromPort) {
	...
	if (fromPort != 7 && infoBase) {
		infoBase->addRequest(to, fromPort);
	}
